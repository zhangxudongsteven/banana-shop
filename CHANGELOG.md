...
- **统一创作暗房视觉语言**：重构深浅主题 token，新增暗房背景、工作台表面、胶片轨道和 amber/cyan 状态样式，替换原先偏通用的黑橙 SaaS 观感。
- **重塑首页与登录入口**：首页改为可感知的创作工作台预览，突出输入图、Prompt、风格胶片带和结果对比；登录页同步为受保护工作台门禁面板。
- **升级 Dashboard 转换选择页**：转换卡片改用示例图预览、能力描述和输入类型标签，支持单图、双图、参考图、文生图、视频、蒙版和两步生成等信息展示。
- **优化创作页结果焦点**：输入区按 Provider、Prompt、图片输入和生成动作组织，结果区放大并强化并排/滑杆对比，提示词详情降级为可展开区域。
- **同步工作台共享表面样式**：Dashboard 顶栏、历史面板、上传器、结果展示和基础按钮/卡片/输入框同步使用新的暗房设计系统。

### 修复

- **修复转换卡片描述换行**：覆盖按钮基础样式中的 `whitespace-nowrap`，确保转换卡片描述可以正常两行截断。

## [0.6.1] - 2026-07-06

### 变更

- **重塑首页创作入口**：将首页改为面向 Dashboard 工作流的产品预览，突出上传、选择风格、生成复用的闭环。
- **统一按钮与图标样式**：Dashboard 顶栏、创作页、结果展示、上传器、历史面板和预览弹窗改用共享 `Button` 与 `lucide-react` 图标，提升交互一致性。
- **整理首页路由引用**：将登录、退出和文档中的首页入口统一为 `/`，移除旧 `/home` 说明。

### 修复

- **增加 API Key 撤销确认**：撤销个人 API Key 前增加确认步骤，避免误操作。
- **改进预览与历史面板可访问性**：为预览弹窗和历史面板补充 dialog 语义、Escape 关闭、焦点恢复，并确保关闭态历史面板不会暴露给键盘导航或辅助技术。
- **补充表单自动填充属性**：登录页账号、密码、手机号和验证码输入补充 `name`、`autoComplete` 与移动端输入提示。
