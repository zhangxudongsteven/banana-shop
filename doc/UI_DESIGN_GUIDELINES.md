...
- 产品定位：AI 图片生成与编辑工具，核心体验围绕 Dashboard 创作工作区，而不是营销页或通用 SaaS 管理后台。
...
使用 `frontend-design` 时，先明确本次 UI 变更服务的创作任务：上传图片、选择转换、输入 Prompt、多图参考、文本生成图片、结果对比、历史复用，或它们之间的某个流程。

设计原则：

- 第一屏应优先呈现可操作的创作工作区，不做纯介绍型 Hero。
- Dashboard 的信息层级以“输入、生成控制、结果预览、历史复用”为核心。
- 图片和生成结果是主视觉，装饰性背景、抽象图形和大面积营销式排版应克制使用。
- 空状态要指向下一步动作，例如上传图片、输入 Prompt、选择转换，而不是只表达情绪。
- 加载状态要区分普通生成、两步生成等已有任务阶段。
...
- 图片上传、Prompt、模型/服务商选择、比例选择都属于输入控制，不应散落到结果区。
...
- 图片生成、图片编辑、图片分析继续通过 `actions/image-actions.ts` 暴露。
...
- 当前只提供文生图、整图编辑、多图参考和两步生成；视频生成、蒙版和选区抠图已下线。旧视频记录不展示，云端数据不删除。
- 生成中锁定所有输入、历史复用与继续编辑，保留旧结果的预览和下载。生成失败不得清空上次结果。
- 结果必须绑定提交时的原图快照；修改下一版输入不能改变上次结果的对比原图。
- 所有继续编辑统一进入自定义改图；远程输入先加载并校验，成功后才替换输入，失败保留原输入及重试入口。
- 上传支持键盘、点击和拖拽；接受可解码的 PNG/JPEG/WebP，每张最多 8 MiB、两张合计最多 16 MiB。
- 保存历史失败允许续传；创建结果不确定时先核对状态，不能盲目新建。未保存结果仅在当前会话保留，应提示先下载。
