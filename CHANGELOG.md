...
- **聚焦图像创作**：移除视频生成入口、服务商能力、MCP 工具、新建 API Key 视频权限和视频历史展示；旧视频 API 返回 HTTP 410，旧视频页面跳转工具列表，云端旧记录及文件不删除。
- **下线局部选区编辑**：移除蒙版绘制与选区抠图入口。编辑接口兼容空蒙版，非空蒙版明确报错且不调用模型；旧图像历史蒙版附件仍可查看。
- **统一图像创作闭环**：所有结果可继续进入自定义改图；生成期间锁定输入、阻止重复提交，保留上一版结果供预览、对比和下载；比较原图绑定该次提交快照。
- **完善上传校验**：支持键盘、点击和拖拽上传，仅接受可解码的 PNG/JPEG/WebP，每张最多 8 MiB、双图合计最多 16 MiB。远程历史图片规范化后复用，失败保留当前输入。

### 修复

- **恢复历史保存**：刷新合并云端与会话记录；保存失败可重试已有图片，已知任务只补传缺失附件。创建结果不确定时按请求 ID 核对，未确认前不重新创建；只有云端 completed 显示已保存。
- **修复并发状态覆盖**：刷新确认完成后不被迟到的失败响应降级，刷新发现的任务 ID 在保存请求失败后仍用于续传。
- **修复移动端对比与下载处理**：保证滑杆画面高度；远程下载校验响应并转为 Blob，按实际图片类型设置扩展名，错误提供提示。

### 测试与文档

- 新增 Vitest、React Testing Library、jsdom 测试，以及仅本地运行的组件预览，覆盖生成锁定、结果保留、输入复用、上传限制和 Tale 保存恢复。
- 更新中英文界面、API/MCP 示例、环境变量说明和项目 README；验收结果及真实服务验证边界见 [图像创作优化验证](doc/IMAGE_WORKFLOW_VALIDATION.md)。
- 待保存载荷仅在当前会话保留；硬刷新前提示先下载。SDK 不支持原子幂等，不承诺跨实例严格去重。
